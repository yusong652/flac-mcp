...
class TestLoadCommandDocVersioned:
    ...
    def test_version_specific_fields_merged_to_top_level(self):
        doc = CommandLoader.load_command_doc("ball", "create", "7.0")
    ...
    def test_unknown_version_raises_key_error(self):
        with pytest.raises(KeyError, match="5.0"):
            CommandLoader.load_command_doc("ball", "create", "5.0")

    def test_version_6_0_loads_correctly(self):
        doc = CommandLoader.load_command_doc("ball", "create", "6.0")
        assert doc is not None
        assert doc["command"] == "ball create"
        assert "syntax" in doc
        assert isinstance(doc["keywords"], list)
    ...
    def test_unknown_command_returns_none(self):
        result = CommandLoader.load_command_doc("ball", "nonexistent_cmd", "7.0")
        assert result is None

    def test_unknown_category_returns_none(self):
        result = CommandLoader.load_command_doc("nonexistent_cat", "create", "7.0")
        assert result is None

    def test_default_version_is_7_0(self):
        doc_default = CommandLoader.load_command_doc("ball", "create")
        doc_explicit = CommandLoader.load_command_doc("ball", "create", "7.0")
    ...
    def test_model_category_versioned(self):
        ...
        doc = CommandLoader.load_command_doc("model", "new", "7.0")
        assert doc is not None
        assert "command" in doc
        assert "syntax" in doc
